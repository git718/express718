...
const {binary,image,string} = javascriptBinaryConverter.utils
const {  integerToBinary,binaryToInteger  } = binary;
...
export function getBytes() {
    const bytes = extraSmallImageDecimalBytes.map(decimal => padString(integerToBinary(decimal)))
    return bytes
}

export function getDecimalBytes(){
    return getBytes().map(str=>binaryToInteger(str))
}
